...
static esp_err_t redirect_to_root(httpd_req_t *req) {
    // Use absolute URL in Location to improve compatibility with some clients
    // that may reject relative redirects when on a captive portal.
    ...
    // that don't follow 302 Location properly in captive-portal situations.

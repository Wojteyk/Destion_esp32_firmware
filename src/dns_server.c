/*
 * Minimal DNS responder: listens on UDP/53 and replies to any query with
 * an A record pointing to 192.168.4.1 (default ESP32 softAP IP).
 * Note: This is a small, non-complete DNS implementation intended for captive
 * portal behavior. It answers only simple A queries and ignores many edge cases.
 */
...
static int parse_qname(const uint8_t *buf, int offset, char *out, int outlen) {
    int i = offset;
    int pos = 0;
    while (buf[i] != 0 && i < 512) {
        uint8_t len = buf[i];
    ...
    return i + 1; // position after the trailing 0
}
...
static void dns_task(void *arg) {
    ...
    while (1) {
        ...
        if (wpos + 16 > (int)sizeof(buf)) continue;
        buf[wpos++] = 0xc0;
        buf[wpos++] = 0x0c;
        ...
        buf[wpos++] = 0x01;
        buf[wpos++] = 0x00; buf[wpos++] = 0x00; buf[wpos++] = 0x00; buf[wpos++] = 0x3c; // TTL 60s
        buf[wpos++] = 0x00; buf[wpos++] = 0x04; // RDLENGTH 4
        // RDATA
